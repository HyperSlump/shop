...
export default function ThemeToggle() {
    ...
    if (!mounted) return <div className="h-8 w-14 rounded-full border border-border/60 bg-card/40" />;
    ...
    return (
        <motion.button
            ...
            aria-pressed={isDark}
            className="relative h-8 w-14 rounded-full border border-border/70 bg-card/70 p-1 backdrop-blur-sm transition-colors duration-200 hover:border-primary/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
        >
            <motion.span
            ...
            <motion.span
                className="relative z-10 flex h-6 w-6 items-center justify-center rounded-full border border-border/80 bg-background text-foreground shadow-[0_1px_4px_rgba(0,0,0,0.22)]"
                animate={{
                    x: isDark ? 0 : 24,
                    rotate: isDark ? -12 : 12
                }}
